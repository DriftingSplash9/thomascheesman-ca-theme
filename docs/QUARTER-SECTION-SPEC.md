...
| **3D-P1.5 — signs & the family gate** | Floating labels replaced with lantern-lit eye-level SIGNPOSTS at each yard, facing their road (clickable). The treehouses now live in a FENCED COMPOUND with one gated road: a barrier arm that lifts for family (`tcVentures.bqFamily` ← `tc_user_is_family()`, page-cache-safe since family are logged in) and sends everyone else to /family-login. Roads rerouted around the grove. | ✅ 1.0.675 |
| **3D-P2 — life & juice** | Dust off the rear wheels (speed/steer-scaled), fading tire tracks, synthesized engine hum + H-key honk (WebAudio, OFF default, persisted toggle top-left), chimney smoke on farmhouse + cookshack, raised mailbox flag (wires to real ledger data later), nocturnal pond fix (dark teal + moon glint). **Deferred from P2:** billboards (quote selection is a content decision with Thomas) + the art pass (proportions/textures/AI-GLB swaps). | ✅ 1.0.679 |
| **3D-P2.5 — the playground** | World 4× (2560×1440), 27-segment twisting road net at width 60, gaussian-heightfield ROLLING HILLS (displaced ground mesh; downhill boosts, uphill drags), 3 dirt-jump MOUNDS with real ballistic air + landing dust, 4 TURBO PADS (speed burst + FOV kick + whoosh), 2 topple-able BALE STACKS + a park-to-RESTACK pad, and a 20-TOKEN hunt (HUD counter, localStorage persist, 2 air-only tokens, find-all fanfare). | ✅ 1.0.680 |
| **3D-P2.6 — monster quarter** | World ×3 again (4480×2520, 12× the original), TILTED grade + hills ~50% stronger; ponds become DIMPLES (negative-gaussian bowls with real water — wade through with splash + drag); MONSTER AIR: Space bunny-hops, L/R Shift pitch mid-air for full FLIPS (clean flip → boost reward, botched → dirt), YZX-euler buggy pitch; 8 HERITAGE DRIVE-IN screens on the perimeter fence (marquee canvas, one per family line, clickable → long-reads). | ✅ 1.0.683 |
